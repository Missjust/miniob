...
{
  // assert(value_num <= sizeof(inserts->row_values[0]) / sizeof(inserts->row_values[0][0]));
  inserts->relation_name = strdup(relation_name);
  inserts->row_num = row_num;
  for(size_t j = 0;j<MAX_ROW_NUM;j++){
    for (size_t i = 0; i < MAX_NUM; i++) {
      inserts->row_values[j][i] = row_values[j][i];
    }
  }
}
void inserts_destroy(Inserts *inserts)
{
  ...
  for(size_t j = 0;j<MAX_ROW_NUM;j++){
    for (size_t i = 0; i < MAX_NUM; i++) {
      value_destroy(&inserts->row_values[j][i]);
    }
  }
}
